...
module.exports = (Ferdium, settings) => {
  ...
  const insertDom = () => {
    ...
    const parent = document.querySelector("aside.css-1dbjc4n")
    if (!parent) {
      return;
    }
...
// 分派 input 事件
  ...
  const clickSendBtn = () => {
    let sendBtn = document.querySelector('div[data-testid="dmComposerSendButton"]');
  ...
  const newMessages = () => {
    let peerIdDom = document.querySelector('div[data-testid="DmActivityContainer"] div[data-testid="cellInnerDiv"] a[role="link"]')
    if (!peerIdDom) {
      return;
    }
    let peerId = peerIdDom.getAttribute("href")
    let rows = document.querySelectorAll('div[data-testid="DmActivityContainer"] div[data-testid="cellInnerDiv"] [data-testid="messageEntry"] [data-testid="tweetText"]');
  ...
  const lastMessages = (num = 10) => {
    ...
    for (const row of rows) {
      ...
      newMessages.push({
        message: text,
        type: row.parentElement.classList.contains('r-l5o3uw') ? 1 : 0
      })
    }
