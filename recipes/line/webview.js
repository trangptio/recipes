...
module.exports = (Ferdium, settings) => {
  ...
  const insertDom = () => {
    ...
    const parent = document.querySelector(".actionGroup-module__action_box__-HA8N")
    if (!parent) {
      return;
    }
...
<svg class="h-3 w-3" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 383.748 383.748" xml:space="preserve"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <path d="M62.772,95.042C90.904,54.899,137.496,30,187.343,30c83.743,0,151.874,68.13,151.874,151.874h30 C369.217,81.588,287.629,0,187.343,0c-35.038,0-69.061,9.989-98.391,28.888C70.368,40.862,54.245,56.032,41.221,73.593 L2.081,34.641v113.365h113.91L62.772,95.042z"></path> <path d="M381.667,235.742h-113.91l53.219,52.965c-28.132,40.142-74.724,65.042-124.571,65.042 c-83.744,0-151.874-68.13-151.874-151.874h-30c0,100.286,81.588,181.874,181.874,181.874c35.038,0,69.062-9.989,98.391-28.888 c18.584-11.975,34.707-27.145,47.731-44.706l39.139,38.952V235.742z"></path> </g> </g></svg>
    ...
    document.querySelector('#text_input').addEventListener(
        ...
        async event => {
          ...
          if (key === 'Enter') {
            ...
            let msgDom = document.querySelector("textarea-ex");
            console.log('sendMsg', msgDom)
            if (msgDom && msgDom.shadowRoot.querySelector("textarea").value) {
              clickSendBtn();
  ...
  const handleSendMessage = async (msg) => {
    ...
    msgDom.focus();
    //
    // msgDom.shadowRoot.querySelector("textarea").focus()
    // msgDom.shadowRoot.querySelector("textarea").value = msg
    // msgDom.shadowRoot.querySelector("div").innerHTML = msg
    // document.querySelector("textarea-ex").insertValue([""])
    const event = new CustomEvent('insertvalue', { detail: [msg] });
    msgDom.dispatchEvent(event);
    document.execCommand("insertText", !1, [msg])


  };
  const clickSendBtn = () => {
    const msgDom =  document.querySelector("textarea-ex");
    msgDom.focus();
// 创建回车键事件
    ...
    msgDom.dispatchEvent(enterEvent)
  };
  const newMessages = () => {
    let peerIdDom = document.querySelector(".chatroom-module__chatroom__eVUaK")
    if (!peerIdDom) {
      return;
    }
    let peerId = peerIdDom.dataset.mid;
    let rows = document.querySelectorAll(".message_list .message-module__message__7odk3");
    for (const row of rows) {
      const trans = row.querySelector('.trans')
      // console.log(trans)
      if (!trans) {
        const contentDom = row.querySelector(".message-module__content__OuUCi")
        const messageDom = contentDom.querySelector('.textMessageContent-module__text__EFwEN');
        if (!contentDom || !messageDom) {
          continue;
        }
        const text = messageDom.textContent
        const tranKey = `trans@${peerId}@${text}`;
        const tranCache = localStorage.getItem(tranKey);
        if (!tranCache) {
          contentDom.insertAdjacentHTML('afterend', '<div class="trans">翻译中...</div>')
          ...
          contentDom.insertAdjacentHTML('afterend', '<div class="trans">' + tranCache + '</div>')
  ...
  const lastMessages = (num = 10) => {
    ...
    for (const row of rows) {
      ...
      newMessages.push({
        message: text,
        type: row.dataset.direction === 'reverse' ? 1 : 0
      })
    }
    return {messages: newMessages.reverse().slice(-1 * num)}
